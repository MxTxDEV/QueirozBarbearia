...
export async function runDemoSeed() {
  ...
  for (const s of SERVICES) {
    const id = `${DEMO_PREFIX}svc-${s.slug}`;
    services.push(
      await prisma.service.upsert({
        where: { id },
        update: { name: s.name, description: s.description, price: s.price, durationMinutes: s.durationMinutes },
        create: { id, name: s.name, description: s.description, price: s.price, durationMinutes: s.durationMinutes },
      })
    );
  }
  ...
  for (let i = 0; i < CUSTOMERS.length; i++) {
    ...
    customers.push(
      await prisma.customer.upsert({
        ...
        update: {},
        create: {
          id: `${DEMO_PREFIX}cus-${String(i).padStart(3, "0")}`,
          fullName,
          whatsapp,
  ...
  for (let offset = -45; offset <= 10; offset++) {
    ...
    for (const barber of barbers) {
      ...
      for (let n = 0; n < perDay; n++) {
        ...
        const id = `${DEMO_PREFIX}apt-${String(appointmentSeq++).padStart(5, "0")}`;
        ...
        if (status === "COMPLETED") {
          const paymentId = `${DEMO_PREFIX}pay-${id.slice(DEMO_PREFIX.length + 4)}`;
          const method = PAYMENT_METHODS[Math.floor(random() * PAYMENT_METHODS.length)];
          await prisma.payment.upsert({
            where: { id: paymentId },
            update: {},
            create: { id: paymentId, appointmentId: id, customerId: customer.id, amount: price, paymentMethod: method, paidAt: endTime },
          });
          await prisma.financialTransaction.upsert({
            where: { id: `${DEMO_PREFIX}ftx-${id.slice(DEMO_PREFIX.length + 4)}` },
            update: {},
            create: {
              id: `${DEMO_PREFIX}ftx-${id.slice(DEMO_PREFIX.length + 4)}`,
              type: "INCOME",
              category: "Serviços",
...
export async function removeDemoData() {
  const demoId = { id: { startsWith: DEMO_PREFIX } };
  ...
  const appointmentServices = await prisma.appointmentService.deleteMany({
    where: { appointmentId: { startsWith: DEMO_PREFIX } },
  });
  ...
  const barberServices = await prisma.barberService.deleteMany({
    where: { serviceId: { startsWith: DEMO_PREFIX } },
  });
