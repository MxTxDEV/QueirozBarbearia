 ...
 * mesmas tabelas. Toda consulta é filtrada por companyId, vindo sempre
 * da sessão autenticada (nunca de input do cliente).
 */
...
export async function getCustomerInsights(companyId: string, period: DashboardPeriod, barberId?: string): Promise<CustomerInsights> {
  ...
  if (servedIds.length > 0) {
    const priorVisits = await prisma.appointment.findMany({
      where: { companyId, customerId: { in: servedIds }, status: "COMPLETED", appointmentDate: { lt: from } },
      select: { customerId: true },
      distinct: ["customerId"],
    });
